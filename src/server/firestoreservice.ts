import { 
  ...
  export const createSubDocument = async (collectionName: string, subCollectionName: string, data: any) => {
    ...
    if (docRef.length === 0) {
      throw new Error(`No document found in ${collectionName} with uniqueID`);
    }
  ...
  export const readSubDocument = async (
    ...
    fieldName: string,
    subDocFieldName: string,
    fieldValue: string
  ) => {
    const docRefs = await readDocument(collectionName, fieldName);
    
    if (docRefs.length === 0) {
      throw new Error(`No documents found in collection ${collectionName} with ${fieldName}`);
    }
  ...
  export const readSubCollection = async (collectionName: string, subCollectionName: string) => {
    ...
    if (docRefs.length === 0) {
      throw new Error(`No documents found in collection ${collectionName} with uniqueID`);
    }
  ...
  export const deleteDocument = async (collectionName: string, subCollectionName: string, docId: string) => {
    ...
    if (parentDoc.length === 0) {
      throw new Error(`No parent document found in ${collectionName} with uniqueID`);
    }
